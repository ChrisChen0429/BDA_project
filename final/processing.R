...
plot_ppcs <- function(sm, sims, inputs, legend='y_rep'){
  ...
  g_2 <- ggplot(df, aes(x=T_rep)) +
    ...
    ylab('') +
    xlab('Defaults') +
    ggtitle('Highest City Default Total')
...
plot_states <- function(state_num, y_state_rep, df_state, inputs){
  T_rep <- y_state_rep[,state_num]
  T_obs <- df_state$y_state[state_num]
  df = data.frame(T_rep=T_rep)
  g1 <- ggplot(df, aes(x=T_rep)) + 
    xlab(sprintf(unique(inputs$state)[state_num])) + 
    geom_histogram(fill='lightblue',
                   color='black',
                   bins = 20) +
    geom_vline(xintercept = T_obs, color = 'red') +
...
multiplot <- function(sims, inputs = inputs, df_state = df_state) {
  y_state_rep <- state_rep(sims = sims, inputs = inputs)
  # plot_states(y_state_rep = y_state_rep, df_state = df_state)
  
  glist = list()
  for (i in 1:32){
    glist[[i]] <- plot_states(state_num = i, inputs = inputs,
                              y_state_rep = y_state_rep, df_state = df_state)
  }
  
  # multiplot(glist, cols = 6)
  gridExtra::grid.arrange(glist[[1]], glist[[2]], glist[[3]], 
                          glist[[4]], glist[[5]], glist[[6]],
                          glist[[7]], glist[[8]], glist[[9]],
                          glist[[10]], glist[[11]], glist[[12]],
                          glist[[13]], glist[[14]], glist[[15]],
                          glist[[16]], glist[[17]], glist[[18]],
                          glist[[19]], glist[[20]], glist[[21]],
                          glist[[22]], glist[[23]], glist[[24]],
                          glist[[25]], glist[[26]], glist[[27]],
                          glist[[28]], glist[[29]], glist[[30]],
                          glist[[31]], glist[[32]], nrow = 8, ncol = 4)
}

plot_descriptive <- function(data){
  g1 <- ggplot(data = data, mapping = aes(x = client_income)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'Avg City Income',
         subtitle = '') +
    ylab(NULL) +
    xlab('log(MXN)') +
  ...
  g2 <- ggplot(data = data, mapping = aes(x = appraisal_value)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'Avg City Mortgage Debt',
         subtitle = '') +
    ylab(NULL) +
    xlab('log(MXN)') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g3 <- ggplot(data = data, mapping = aes(x = app_2_inc)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'Avg. Mortgage to Income Ratio',
         subtitle = '') +
    ylab(NULL) +
    xlab('Numeric') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g4 <- ggplot(data = data, mapping = aes(x = mar_2_app)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'Avg Market value to Mortgage Ratio',
         subtitle = '') +
    ylab(NULL) +
    xlab('Numeric') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g5 <- ggplot(data = data, mapping = aes(x = sex_F)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'City Female %',
         subtitle = '') +
    ylab(NULL) +
    xlab('%') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g6 <- ggplot(data = data, mapping = aes(x = age)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'Avg. City Age',
         subtitle = '') +
    ylab(NULL) +
    xlab('Years') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g7 <- ggplot(data = data, mapping = aes(x = risk_index)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'Avg. City Risk Index',
         subtitle = '') +
    ylab(NULL) +
    xlab('Numeric') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g8 <- ggplot(data = data, mapping = aes(x = employed_30)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'City Unemployed %',
         subtitle = '%') +
    ylab(NULL) +
    xlab('') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
  ...
  g9 <- ggplot(data = data, mapping = aes(x = condition_U)) +
    geom_histogram(fill='lightblue', color='black', bins = 30) +
    labs(title = 'City % of new houses',
         subtitle = '%') +
    ylab(NULL) +
    xlab('') +
    theme(plot.title = element_text(size = 10, face = "bold")) +
