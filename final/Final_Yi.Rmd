...
$$ y_i \sim Binormial(n_{i},a + x_i\beta) $$
As a robust prior distribution option, we take $\beta \sim cauchy(location=0,scale=2.5)$.

In model 2, we will extend this model in two ways: (1) incoperate the geographic state information,(2) hierarchical extension for intercept terms, (3) zero inflated model.  
...
In  our model, we assume the hyperparameter $\tau \sim Gamma(2,2)$. We define the sparse_iar_lpdf function as a more efficient spare representation as the following:
...
### Extension two: Hierarchical extension for intercept terms

In our model, we let the intercept term $a$ in our model base on the IAR prior $\phi$. At the same time, we take the cauchy distribution to let the model to be more robust. Since the a is the intercept term in the binormail regression. We assume the scale to be 10.  For the $j$ th city in the $i$ the state, we have:

$$a_{ij} \sim cauchy(location=\phi_i,scale= 10)$$
In this way, $a$ is the state-level effect which represent the overall effects from the factors we have not taken as independent variables. There are two resons we do not take the hierarchical extension for the coefficient terms.

1. Hierarchical extension for the $\beta$s will greatly expend the demension of the parameter space. Originally, we have 1 intercept and 7 coefficient in the binormial regression model (total 8 parameters). If we take the hierarchical extension both on the intercept and coefficient terms, there will have $32 \times 1 = 32$ intercept paramters and $32 \times 7 = 224$ coefficient parameters (total 256 parameters). Now, we have$32 \times 1 = 32$ intercept paramters and 7 coefficient parameters (total 39 parameters). This is more acceptable.

2. Now the coefficient terms are not depend on the state prior. Thus, we can estimate the overall effect from these independent variables, like age and income.

### Extension three: zero inflated model
...
We assume a parameter $\theta$ as the probability of drawing a zero and the probability $1-\theta$ as drawing fro mthe Binormal distribution. The distribution function is thus:

$$p(y_n | \theta,a,\beta) =\begin{cases} 
\theta + (1-\theta) \times Binomial(0 |a,\beta ) & y_n =0 \\
(1-\theta) \times Binomial(0 |a,\beta )  & y_n > 0 \end{cases}$$
...
And we predict the $y_rep$ in the following way:
...
In order to determine our model performance, again we do the cross validation. And we calculate the MSE for our model. The stan code we used to simulate the $y_hat$ is as the following:
...
And it shows that the cross validation for model 2 is : **10020**. 
...
for (i in 1:5){
  a <- c(1,2,3,4,5)[-i]
  input_test = splited_inputs[[i]]
  input_train = rbind(splited_inputs[[a[1]]] ,splited_inputs[[a[2]]],splited_inputs[[a[3]]],splited_inputs[[a[4]]]) 

  y_train = input_train$sum_y
  y_test = input_test$sum_y
...
## Inputs for STAN
  ...
  name = paste('model2_cv',str(i),'.rds')
