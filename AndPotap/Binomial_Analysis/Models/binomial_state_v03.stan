data {
  ...
  for (s in 1:S) {
    ...
    for (d in 1:D) {
     beta_s[d, s] = beta[d] + beta_s_raw[d, s] * sigma_s_beta[s]; 
    }
  }
} model {
  alpha ~ normal(0, 5);
  ...
  beta ~ normal(0, 5);
  for (d in 1:D){
   beta_s_raw[d, :] ~ std_normal(); 
  }
  y_train ~ binomial_logit(State_N_train,
  alpha_s[State_train] + 
  rows_dot_product(X_train, beta_s[: ,State_train]));
  //y_train ~ binomial_logit(State_N_train,
  //alpha_s[State_train] + 
  //X_train * col(beta_s, State_train));
} generated quantities {
  int<lower=0> yrep_test[N_test];
  for (i in 1:N_test) {
    yrep_test[i] = binomial_rng(State_N_test[i], 
    inv_logit(alpha_s[State_test[i]] + 
    X_test[i, :] * beta_s[, State_test[i]]));
  }
}
