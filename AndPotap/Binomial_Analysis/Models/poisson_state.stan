data {
  ...
  beta ~ normal(0, 5);
  y_train ~ poisson(to_vector(State_N_train) .* 
  exp(alpha_s[State_train] + X_train * beta));
} generated quantities {
  int<lower=0> yrep_test[N_test];
  for (i in 1:N_test) {
    yrep_test[i] = poisson_rng(State_N_test[i] *
    exp(alpha_s[State_test[i]] + X_test[i, :] * beta));
  }
}
